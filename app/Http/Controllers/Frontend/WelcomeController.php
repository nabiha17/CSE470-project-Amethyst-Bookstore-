...
class WelcomeController extends Controller
{
    ...
    public function viewBook($slug){
        ...
        $settings = DB::table('settings')->where('id', 1)->get()[0];
        return view('front.book.index', compact('book', 'settings'));
    }
...
//        $orders = Order::where()
    ...
    public function publisher_reg(){
        $settings = DB::table('settings')->where('id', 1)->get()[0];
        return view('front.auth.publisher_reg', compact('settings'));
    }
    ...
    public function authorIndex(){
        $settings = DB::table('settings')->where('id', 1)->get()[0];
        $authors = Author::all();
        return view('front.authors', compact('authors', 'settings'));
    }
    public function publisherIndex(){
        $settings = DB::table('settings')->where('id', 1)->get()[0];
        $publishers = Publisher::where('is_active', 1)->get();
        return view('front.publisher', compact('publishers', 'settings'));
    }
    public function categoryIndex(){
        $settings = DB::table('settings')->where('id', 1)->get()[0];
        $categories = Category::all();
        return view('front.categories', compact('categories', 'settings'));
    }

    public function bookList(){
        $settings = DB::table('settings')->where('id', 1)->get()[0];
        $books = Book::where('status', 1)->paginate(30);
        return view('front.bookgrid', compact('books', 'settings'));
    }
